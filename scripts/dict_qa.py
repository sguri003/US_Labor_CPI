...
def read_st():
    ...
    if os.path.exists(PROJECT_ROOT / 'new_states.json'):
        os.remove(PROJECT_ROOT / 'new_states.json')
        print('File removed')
    else:
        print('Creating File')
    with open(PROJECT_ROOT / 'new_states.json', 'w') as f:
        json.dump(my_dict, f, indent=4, sort_keys=True)
    #iterate over all states in JSON sorted
    for k, v in my_dict.items():
        print(k + str(v))
    with open(DATA_DIR / 'JSON_CSV.csv', 'w', newline='') as csvfile:
...
def read_j():
    if os.path.exists(PROJECT_ROOT / 'state_format.json'):
        os.remove(PROJECT_ROOT / 'state_format.json')
        print('File exists writing JSON dump')
    else:
        print("No file created")
    my_d = {}
    with open(PROJECT_ROOT / 'states.json' ,'r') as st_j:
        my_d = json.load(st_j)
        #print(my_d)
    with open(PROJECT_ROOT / 'state_format.json' , 'w') as j_df:
        json.dump(my_d,j_df, indent=4, sort_keys=False)

    dt = pd.read_json(PROJECT_ROOT / 'states.json')
